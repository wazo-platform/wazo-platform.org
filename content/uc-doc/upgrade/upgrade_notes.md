...
- Support for legacy patterns of pre-dial handlers in dialplan config have been deprecated since 21.01 in favor of `wazo-add-pre-dial-hook`.
  Automatic conversion of the legacy pattern to the new pattern is now removed, and any remaining legacy configuration will now have to be updated accordingly. See [pre-dial handlers](/uc-doc/api_sdk/subroutine) and [21.01 release notes](.#21.01) for more details.
