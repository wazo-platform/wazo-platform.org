...
const Page = ({ plugins, images }: Props) => {
  ...
  const onDeviceClick = (event) => {
    ...
    setCurrentDevice(
      currentDevice === event.target.id ? null : event.target.id,
    );
  };
  ...
  const onVendorClick = (event) => {
    event.preventDefault();
    setCurrentVendor(
      currentVendor === event.target.id ? null : event.target.id,
    );
  };
  ...
  return Object.keys(plugins).map((vendor) => (
    <div className="vendor" key={vendor}>
      <a
        className="title"
        href={`/provisioning/${slugify(vendor)}`}
        id={vendor}
      ...
      {vendor === currentVendor && (
        <div className="phones">
          {Object.keys(plugins[vendor])
            .sort()
            .map((name) => {
              const id = `${slugify(vendor)}-${slugify(name)}`;
              return (
                <div
                  className={id === currentDevice ? 'selected' : ''}
                  key={id}
                >
                  <a
                    href={`/provisioning/${slugify(vendor)}/${slugify(name)}`}
