---
title: Debian 12 (Bookworm) Upgrade Notes
---

The upgrade to Wazo 25.15 or later will take longer than usual, because the whole Debian system will
be upgraded.

The database management system (PostgreSQL) will also be upgraded from version 13 to version 15 at
...
- Make sure your version of Wazo is at least 23.06. You can run `wazo-upgrade` to check the version
...
- If you have customized the Debian system of your Wazo in some nontrivial way, you might want to
  ...
  - Make sure you don't have any unofficial sources in your `/etc/apt/sources.list` or
    `/etc/apt/sources.list.d` directory. If you were using the `bullseye-backports` source, you must
    remove it.
...
- `wazo-dist -m pelican-bullseye`: Ensures your system is not restricted to a specific version
- `wazo-upgrade`: Installs the `wazo-dist-upgrade` script and makes sure the system is up-to-date.
- `wazo-dist-upgrade`: Upgrades to the latest version of Wazo with Debian 12 (Bookworm). This
