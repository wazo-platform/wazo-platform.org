...
const Page = () => {
  ...
  const getListItemClasses = (currentClass, itemKey) => {
    ...
    return [currentClass, openClassName, currentClassName]
    ...
    if (subLinksKeys.length >= 2) {
      ...
      return (
        <li key={itemKey} className={getListItemClasses('secondary-navigation-submenu', itemKey)}>
          <a href={self.path}>
            ...
            <span
              onClick={e => handleMenuClick(e, itemKey)}
              className="secondary-navigation-submenu-icon"
            ></span>
          </a>
          <ul>
      ...
      return (
        <li key={itemKey} className={getListItemClasses('', itemKey)}>
          <Link to={linksObject[subLinksKeys[0]].path}>
            {formatLinkTitle(linksObject[subLinksKeys[0]].title)}
          </Link>
        </li>
      );
    }
  ...
  const renderLinks = () => {
    return (
      <ul>
        <li>
          <Link to="/uc-doc/">Wazo Documentation</Link>
        </li>
