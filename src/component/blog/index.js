      ...
      <div className="container">
        {filter?.type && (
          <div className="filter">
            Filtering by {filter?.type}: {filter?.value}{' '}
            <Link className="reset-filter" to="/blog" state={{ filter: {} }}>
              Reset filter
            </Link>
          </div>
        )}
        <div className="articles">
          {articles.map(
            ({ title, slug, date: dateRaw, author, category, tags: tagsRaw, summary }) => {
                        ...
                        * Tagged with{' '}
                        {tags.map(item => (
                          <Link
                            ...
                            className="hilite"
                            to="/blog"
                            state={{ filter: { type: 'tag', value: item } }}
                          >
                            {item}
                          </Link>
                        ))}
